...
/*
	...
	Modified to support all version codes.
*/

function armory_extend_chain(pubKey, chainCode, privKey, fromPrivKey, version) {
...
function armory_derive_chaincode(root)
{
  ...
  // Vulnerable to Array.prototype modifications
  for (var i in hash)
  {
...
var Armory = new function () {
    ...
    function calcAddr(version) {
        var r = armory_extend_chain(pubKey, chainCode, privKey, true, version);
        onUpdate(r);
        pubKey = r[2];
        privKey = r[3];
        counter++;
    ...
    this.gen = function(seed, _range, version, update, success) {
